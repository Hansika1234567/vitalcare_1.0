...
if (envLoadResult.error) {
  ...
  console.log("✅ Loaded .env from", envPath, "keys:", Object.keys(envLoadResult.parsed || {}));
}
...
async function startServer() {
  ...
  console.log("Starting VitalCare server on port", PORT);
  ...
  function getGeminiClient() {
    if (!aiClient) {
      ...
      if (!apiKey) {
        console.error("❌ GEMINI_API_KEY is not set. AI features will use fallback mock responses. Please create a .env file with GEMINI_API_KEY=your-key");
        return null;
      }
