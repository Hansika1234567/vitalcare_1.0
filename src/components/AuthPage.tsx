  ...
  const handleDemoLogin = async (selectedRole: UserRole) => {
    ...
    try {
      ...
      const demoCredentials: Record<UserRole, { email: string; password: string; name: string }> = {
        patient: { email: "ab@gmail.com", password: "12345678", name: "Ramesh Kumar (Demo Patient)" },
        caretaker: { email: "c@gmail.com", password: "12345678", name: "Sita Kumar (Demo Caretaker)" },
        doctor: { email: "d@gmail.com", password: "12345678", name: "Dr. Anjali Sharma (Demo Doctor)" },
        admin: { email: "adc@gmail.com", password: "12345678", name: "System Admin (Demo)" }
      };
      ...
      try {
        ...
        userCredential = await signInWithEmailAndPassword(auth, creds.email, creds.password);
      } catch (signInErr: any) {
        // If sign-in fails, try to create the account
        if (signInErr.code === "auth/user-not-found") {
          userCredential = await createUserWithEmailAndPassword(auth, creds.email, creds.password);
        } else {
          throw signInErr;
        }
      }
      ...
      // Save the demo profile to Firestore
      try {
        await setDoc(doc(db, "users", demoUser.uid), demoProfile);
      } catch (firestoreErr) {
        console.warn("Firestore save failed, proceeding with local profile:", firestoreErr);
        // Continue even if Firestore save fails - use local profile
      }
      
      onAuthSuccess(demoProfile);
    } catch (err: any) {
      const errorMsg = err?.message || String(err);
      let friendlyError = "Failed to initialize demo mode. ";
      
      if (errorMsg.includes("auth/invalid-email")) {
        friendlyError += "Invalid email format in demo configuration.";
      } else if (errorMsg.includes("auth/weak-password")) {
        friendlyError += "Demo password is too weak (requires 6+ characters).";
